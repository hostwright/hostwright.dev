...
- selected manifest presence and validity when supplied.
- Local state database availability and schema status.
...
`doctor` runs locally without invoking Apple container. Runtime observation
of containers, networks, volumes, images, and logs lives behind the
[RuntimeAdapter](/concepts/runtime-adapter/). Apple container's version is
...
dimensions; see the [CLI reference](/reference/cli/). Doctor reports only
measured values.
