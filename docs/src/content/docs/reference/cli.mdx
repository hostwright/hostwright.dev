...
redacted lock owner and lease expiry. Diagnostic output only; recovery actions stay manual.
...
an existing bundle. The command reads existing state rows only, and the
bundle stays on local disk.
...
absence. It uses a pre-pulled local image, runs without state writes, and keeps the
report on local disk. Exit 0 only
...
report records local evidence for the exact host and versions measured.
...
records events. Runtime mutation stays with the confirmed apply gate. See [Daemon](/operations/daemon/).
