...
formula, `.pkg`, and signed, notarized binaries) is on the roadmap. Assistants
tend to paper over that gap: they may suggest `brew install hostwright` or
invent a `hostwright down` command, and neither exists today. This page is one prompt that
fixes both.

<Aside type="caution" title="No installer today">
Hostwright runs from Swift Package Manager source. A trusted vendor
tap/archive/`.pkg` and Homebrew-core submission are on the roadmap; those
outcomes are not current installation commands.
</Aside>
