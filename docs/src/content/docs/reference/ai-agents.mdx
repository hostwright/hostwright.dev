...
Safe commands (no runtime mutation): --version, capabilities, paths, state
integrity, migrate preview, init, import-stack, validate, plan, status, logs,
events, recovery, diagnostics, cleanup --dry-run, doctor. State-backed commands
resolve their database from --state-db, then HOSTWRIGHT_STATE_DB, then the
default at ~/Library/Application Support/Hostwright/state/state.sqlite.
